...
describe('CharacterListFilterPipe', () => {
  it('create an instance', () => {
    ...
    const pipe = new CharacterListFilterPipe();
    expect(pipe).toBeTruthy();

    let filtered = pipe.transform(chars, "Luke");
    expect(filtered.length).toBe(1);

    filtered = pipe.transform(chars, "");
    expect(filtered.length).toBe(chars.length);
  });
});
